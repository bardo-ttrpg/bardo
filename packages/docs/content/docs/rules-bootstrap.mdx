...
- If the file uses another name, such as `RULEBOOK.md`, pass `--rulebook ./RULEBOOK.md`.
...
Large rulebooks are not treated as campaign notes. They are handled by rules bootstrap, then skipped during campaign note discovery so they do not create misleading oversized-source readiness gaps.
