...
const OPENCODE_INSTRUCTION_PATHS = [
...
function buildLocalAdapterArgs(_baseUrl: string): string[] {
	return [...LOCAL_ADAPTER_PREFIX_ARGS, "--workspace-root", "."];
}
...
function mergeJsonVariant(args: {
	...
	if (args.variant === "opencode") {
		...
		root.mcp = mcp;
		root.instructions = Array.from(
			new Set([...instructions, ...OPENCODE_INSTRUCTION_PATHS]),
		);
		return root;
	}
