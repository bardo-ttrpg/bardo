...
export async function runSimulationTick(args: {
	auth: AuthContext;
	mode: "turn" | "scheduled";
	tickCount: number;
	...
	const { auth, mode, tickCount, idempotencyKey, dryRun = false } = args;
	...
	return withKeyedLock(`workspace-mutation:${bardoRoot}`, async () => {
		...
		const rng = createDeterministicRng(
			parseSeed(`${idempotencyKey ?? "dry-run"}:${worldTimeBeforeISO}:${mode}`),
		);
		...
		for (let i = 0; i < tickCount; i += 1) {
			...
			const selectedLocation =
				locationKeys.length > 0
					? locationKeys[Math.floor(rng() * locationKeys.length)] || "unknown"
					: "starting-area";
...
export function registerSimulationTickTool(
	...
	server.registerTool(
		...
		async ({ mode, tickCount, idempotencyKey, dryRun }) => {
			try {
				const output = await runSimulationTick({
					auth,
					mode,
					tickCount,
					idempotencyKey,
				...
				const bardoRoot = resolveBardoRoot(auth.campaignBasePath);
				const statePath = resolvePathInsideRoot(bardoRoot, "state/current.md");
				const historyPath = resolvePathInsideRoot(
					bardoRoot,
					"state/history.md",
				);
				const output: SimulationTickOutput = {
