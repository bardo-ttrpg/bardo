...
export function parseStateOrThrow(args: {
	rawBody: string;
	sourcePath?: string;
	allowEmpty?: boolean;
}): CampaignState {
	const { rawBody, sourcePath, allowEmpty = true } = args;
	if (!rawBody.trim()) {
		if (allowEmpty) {
			return newStateTemplate();
		}
		throw new Error(
			`MALFORMED_CAMPAIGN_STATE: ${sourcePath ?? "state"} is empty and cannot be used as persisted canon.`,
		);
	}

	let parsed: Partial<CampaignState>;
	try {
		parsed = JSON.parse(rawBody) as Partial<CampaignState>;
	} catch (error) {
		const message =
			error instanceof Error ? error.message : "invalid JSON payload";
		throw new Error(
			`MALFORMED_CAMPAIGN_STATE: ${sourcePath ?? "state"} contains invalid JSON (${message}).`,
		);
	}
		...
		scene:
			typeof parsed.scene === "object" && parsed.scene !== null
				? {
						...
						unresolvedQuestions: Array.isArray(parsed.scene.unresolvedQuestions)
